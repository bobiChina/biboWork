	...
	$.fn.createTable = function(options){
		...
		var options = $.extend(defaults, options);
			...
			for(var i = 0; i < _bmu.length; i++){
				...
				if(_bmu[i].bmu[options.index] != undefined && _bmu[i].bmu[options.index].hasOwnProperty("singleVoltageList")){
					singleVoltageList = _bmu[i].bmu[options.index].singleVoltageList.split(",");    //定义电压表格个数
				}else{
				...
				if(_bmu[i].bmu[options.index] != undefined && _bmu[i].bmu[options.index].hasOwnProperty("boxTemperList")){
					boxTemperList = _bmu[i].bmu[options.index].boxTemperList.split(",");			//定义温度表格个数
				}else{
				...
				if(_bmu[i].bmu[options.index] != undefined && _bmu[i].bmu[options.index].hasOwnProperty("balanceAmpList")){
					balanceAmpList = _bmu[i].bmu[options.index].balanceAmpList.split(",");			//定义电流表格个数
				}else{
					boxTemperList.push("00");
				}
