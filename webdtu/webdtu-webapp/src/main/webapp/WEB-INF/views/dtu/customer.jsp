...
function updateUser(){
	...
	if (corpMail.length >0){
		...
		if(!reg.test(corpMail)){
			showDialogAlertFocus("邮箱格式不正确", "corpEmailEdit");
			return;
		}
	}
...
function findCurUserDtu(id){
	...
	$.get("${ctx}/dtu/getUserDtuList", {userid:id},function(data){
		...
		$.each(result, function(index, items){
			...
			tr += "<img onclick='delDtuConfigFun(\"" + (items[5]==null || items[5]=="null" ? "" : items[5]) + "\")' src='${ctx}/static/images/del.png' style='position: absolute; top: 0px; right: 0px; width: 20px; cursor: pointer; '>";
			tr += "</div>";
			tr += "</td>";
			tr += "</tr>";
...
function findUserDtuAll(){
	...
	$.get("${ctx}/dtu/getUserDtuAll", {userid:id, uuid:uuid , isUse:99 },function(data){
		...
		$.each(result, function(index, items){
			tr += "<tr>";
			if(1 == items[5]){
				tr += "<td style='width:50px;border: 1px solid #cccccc;'><input type='checkbox' name='selDtuObj' value='" + items[6] + "' checked /></td>";
			}else{
				tr += "<td style='width:50px;border: 1px solid #cccccc;'><input type='checkbox' name='selDtuObj' value='" + items[6] + "' /></td>";
			}
			...
			<div class="table-body page-table-body">
				...
				<div class="historty-table-con-outer dclear">
					<div class="history-table-con-inner manage-page-inner clearfix" style="margin-bottom: 0px !important;">
						...
						<div class="manage-page-right dright" id="userInfoShow" style="margin-bottom: 0px !important; display:none;">
								...
								<div class="table-body2" >
									<table class="dclear" >
										<tr class="trHead">
											...
											<th style="width:170px;">SN/UUID</th>
							...
							<div class="manage-page-left dleft" style="margin-top:0px; margin-bottom: 0px !important;height:360px; width:760px; background-color:#FFF;">
								<div class="table-body2" >
									<table class="dclear" >
