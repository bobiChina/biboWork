(function($){
	...
	$.fn.createTable = function(options){
		...
		var options = $.extend(defaults, options);
			...
			var _voltageNums = _bmu[0].bmu[options.index].singleVoltageList.split(",").length;    //定义电压表格个数
			var _temperNums = _bmu[0].bmu[options.index].boxTemperList.split(",").length;			//定义温度表格个数
			var _ampNums = _bmu[0].bmu[options.index].balanceAmpList.split(",").length;			//定义电流表格个数
			...
			for(var i = 0; i < _bmu.length; i++){
				...
				_eachCreateVoltageTD(_bmu[0].bmu[options.index].singleVoltageList.split(","), _voltageNums);    	//生成电压表格

				_eachCreateTemperTD(_bmu[0].bmu[options.index].boxTemperList.split(","), _temperNums);			//生成温度表格 
				...
				_eachCreateAmpTD(_bmu[0].bmu[options.index].balanceAmpList.split(","), _ampNums);					//生成电流表格

				//加热状态
				if( 1 == _bmu[i].bmu[options.index].hotStatus){
					_tdStr += "<td>开启<\/td>";
				}else{
					_tdStr += "<td>关闭<\/td>";
				}
				...
				//风扇状态
				if( 1 == _bmu[i].bmu[options.index].fanStatus){
					_tdStr += "<td>开启<\/td>";
				}else{
					_tdStr += "<td>关闭<\/td>";
				}
		...
		if(!$(this).find(".history-save-table-head-inner").hasClass("complete-data")){
			...
			if(options.cutTb.length > 0){
				...
				var _lastSpan = parseInt(_tableFixWidth + 50) - parseInt(_notLastSpan);
	...
	$.fn.calendar = function(options){
			...
			var _defaultHtml = "<style type=\"text/css\">"
								...
								+			"<span class=\"js-calendar-year\">2015<\/span>"
								...
								+			"<span class=\"js-calendar-month\">2015<\/span>"
