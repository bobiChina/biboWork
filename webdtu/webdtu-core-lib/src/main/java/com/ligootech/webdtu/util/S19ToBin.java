...
public class S19ToBin {
    ...
    public void downloadZip4Order(String fileCode, ZipOutputStream outputStream) {
        ZipFile zipFile = null;
        try {
            zipFile = new ZipFile(FileOperateUtil.UPLOADDIR + fileCode + ".zip", FileOperateUtil.CHINESE_CHARSET);
            ...
            while (zipEntries.hasMoreElements()) {
                ...
                byte[] buffer = new byte[FileOperateUtil.CACHE_SIZE];
                int r = 0;
     ...
     * @return 新生成文件名
     */
    public String operateS19File(String fileCode, boolean isBcu) {
        BufferedReader br = null;
        BufferedOutputStream bo_json = null;
        String fileName = BhGenerator.getBh() + ".json";
        try {
            bo_json = new BufferedOutputStream(new FileOutputStream(FileOperateUtil.UPLOADDIR + fileName, false));
            //br = new BufferedReader(new InputStreamReader(new FileInputStream(new File(FileOperateUtil.UPLOADDIR + fileCode))));
            /*****************************
             * 修改为压缩文件编号传入，
             * 需先解压后生成文件。
             ****************************/
            ZipFile zipFile = new ZipFile(FileOperateUtil.UPLOADDIR + fileCode + ".zip", FileOperateUtil.CHINESE_CHARSET);
            Enumeration<ZipEntry> zipEntries = zipFile.getEntries();
            ZipEntry entry;
            while (zipEntries.hasMoreElements()) {
                entry = (ZipEntry) zipEntries.nextElement();
     ...
     * @throws FileCodeErrorException
     */
    private Map<String, Object> translationS19(String lineStr, long existingNum, String s2_ppage, OutputStream jsonOutputStream, boolean isBcu) throws IOException, CodeErrorException, FileCodeErrorException {
        int lineLength = lineStr.length();
        Map<String, Object> map = new HashMap<String, Object>();
        logger.debug("原始扫描数据：" + lineStr);
        ...
        if ("S0".equalsIgnoreCase(typeStr) || "S2".equalsIgnoreCase(typeStr) || "S9".equalsIgnoreCase(typeStr) || isBcu) {
            ...
            logger.debug("文件中存在不被允许的数据类型：" + typeStr);
            throw new FileCodeErrorException("文件中存在不被允许的数据类型：" + typeStr);
        }
        ...
        if (existingNum == 0l){
            ...
            if (s2_ppage == null && "S2".equalsIgnoreCase(typeStr)) {
                ...
                if (initCodeNum == 192 || initCodeNum == 207 || initCodeNum == 224 || initCodeNum == 231 ){
                    ...
                    logger.debug("初始化>>>获取到的S2页地址为：" + initCodeStr + "  不存在于指定区间！");
                    throw new CodeErrorException("初始化>>>获取到的S2页地址为："+ initCodeStr + "  不存在于指定区间！");
                }
            }
        }
        ...
        if (isBcu) {
            ...
            boolean codeCheck = checkRegion(codeing_reginStr, s2_ppage);
            if (!codeCheck) {
                logger.debug("获取到的S2页地址为：" + codeing_reginStr + "，检查类型为：" + s2_ppage + "  不存在于指定区间！");
                throw new CodeErrorException("获取到的S2页地址为：" + codeing_reginStr + "，检查类型为：" + s2_ppage + "  不存在于指定区间！\n");
            }
            ...
            if (addressStart < 0x1080 || (addressStart > 0x13FF && addressStart < 0x1900)) {
                logger.debug("获取到的S1地址为：" + addressStart + "  不存在于指定区间！");
        ...
        StringBuffer buff = new StringBuffer();
        if (existingNum == 0l){
            logger.debug("起始位置开始填充");
            jsonOutputStream.write(("{\"start_addr\":" + addressStart).getBytes());
            if (addressStart == 0l){

            }else{
                long diff = addressStart - 1l ;
                logger.debug("首次补充的长度为：" + diff + "  本次开始位置：" + addressStart);
                //首次补充不记录
                /*for (int i = 0; i < diff; i++) {
                    buff.append("FF");
                }*/
            ...
            long diff = addressStart - 1l - existingNum ;
            logger.debug("本次补充的长度为：" + diff + "  上次结束位置：" + existingNum + "  本次开始位置：" + addressStart);
            String dataStr = jsonDataBuilder.toString();
            int len = dataStr.length();
            logger.debug("dataStr = " + dataStr + " len = " + len);
            if (len % 16 != 0) {
                int fill = (len / 16 + 1) * 16 - len;
                logger.debug("after-fill = " + fill + "@" + (addressStart + len / 2 - 1) + "(" + addressStart + " + " + (len / 2 - 1) + ")");
                for (int i = 0; i < fill; i++) {
                    dataStr = dataStr + "F";
                }
            }
            ...
            if (addressStart % 8 != 0) {
                ...
                logger.debug("pre-fill = " + fill + "@" + addressStart);
            ...
            logger.debug("未填充, 起始字节数为：" + addressStart);
        }
        buff.append(dateStr);
